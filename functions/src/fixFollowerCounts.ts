...
export const fixFollowerCounts = functions.https.onCall(async (data, context) => {
  if (!context.auth) {
...
export const fixOrganizationFollowerCount = functions.https.onCall(async (data, context) => {
  if (!context.auth) {
    throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
  }
